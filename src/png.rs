...
impl TryFrom<&[u8]> for Png {
  ...
  fn try_from(value: &[u8]) -> super::Result<Self> {
    let (header, mut value) = value.split_at(8);
    ...
    while value.len() > 0 {
      let (len, _) = value.split_at(4);
      let len = u32::from_be_bytes(len.try_into()?);

      let (chunk_raw, remain) = value.split_at(len as usize + 12);
      chunks.push(Chunk::try_from(chunk_raw)?);
